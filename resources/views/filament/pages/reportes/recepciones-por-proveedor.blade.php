    ...
    <div class="space-y-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($this->recepciones as $recepcion)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2 text-right">${{ number_format($recepcion->impuestos_total, 2) }}</td>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="bg-white rounded-xl p-4 shadow border border-gray-200">
                <p class="text-sm text-gray-500">Total recepciones</p>
                <p class="text-2xl font-bold text-gray-800">{{ $this->totalRecepciones }}</p>
            </div>
            <div class="bg-white rounded-xl p-4 shadow border border-gray-200">
                <p class="text-sm text-gray-500">Monto total</p>
                <p class="text-2xl font-bold text-gray-800">${{ number_format($this->totalMonto, 2) }}</p>
            </div>
        </div>
    </div>
</x-filament-panels::page>
