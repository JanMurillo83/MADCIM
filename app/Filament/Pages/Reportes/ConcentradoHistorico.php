...
class ConcentradoHistorico extends Page
{
    use HasRolePageAccess;

    protected static string|BackedEnum|null $navigationIcon = 'heroicon-o-archive-box';
    protected static ?string $navigationLabel = 'Concentrado Historico';
    protected static ?string $title = 'Concentrado Historico';
    protected static string|null|\UnitEnum $navigationGroup = 'Concentrados';
    protected static ?int $navigationSort = 2;

    protected string $view = 'filament.pages.reportes.concentrado-historico';
    ...
    public function exportPdf()
    {
        $pdf = Pdf::loadView('exports.reportes.concentrado-pdf', [
            'titulo' => 'Concentrado historico de madera enviada al cliente x obra',
            'grupos' => $this->grupos,
        ...
        return response()->streamDownload(function () use ($pdf) {
            echo $pdf->output();
        }, 'concentrado_historico_' . now()->format('Ymd_His') . '.pdf');
    }

    public function exportExcel()
    {
        $tmpPath = tempnam(sys_get_temp_dir(), 'concentrado_historico_xlsx_');
        $filename = 'concentrado_historico_' . now()->format('Ymd_His') . '.xlsx';

        $writer = new XLSXWriter();
        $writer->openToFile($tmpPath);
        $writer->addRow(Row::fromValues(['Concentrado historico de madera enviada al cliente x obra']));
